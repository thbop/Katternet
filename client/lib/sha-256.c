...
// https://github.com/amosnier/sha-2/tree/master
...
void calc_sha_256_s( char result[SHA_256_HASH_RESULT_SIZE], const void *input ) {
    uint8_t hash[SIZE_OF_SHA_256_HASH];
    calc_sha_256(hash, input, strlen(input));

    for ( int i = 0; i < SIZE_OF_SHA_256_HASH; i++ ) {
        result[i*2] = (hash[i] >> 4) < 10 ? (hash[i] >> 4) + '0' : (hash[i] >> 4) - 10 + 'a';
        result[i*2+1] = (hash[i] & 0xF) < 10 ? (hash[i] & 0xF) + '0' : (hash[i] & 0xF) - 10 + 'a';
    }
    result[64] = 0;
}